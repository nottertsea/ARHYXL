...
app.get('/api/account/orders', requireUser, async (request, response, next) => {
    try {
        ...
        response.json(orders.map((order) => ({ ...order, status: order.status === 'paid' ? 'processing' : order.status, has_return: Boolean(order.has_return), items: items.filter((item) => item.order_reference === order.order_reference) })));
...
app.get('/api/admin/orders', requireOwner, async (request, response, next) => {
    try {
        ...
        response.json(orders.map((order) => ({ ...order, status: order.status === 'paid' ? 'processing' : order.status, items: items.filter((item) => item.order_id === order.id) })));
    } catch (error) { next(error); }
});
