...
Email confirmation and password recovery use a Gmail App Password (not your normal login password). Set `GMAIL_USER`/`GMAIL_APP_PASSWORD` on Railway. A generic `SMTP_*` provider is also supported as a fallback if Gmail isn't configured or fails. In production, the backend does not reveal verification or reset links in API responses when no email provider is configured.
